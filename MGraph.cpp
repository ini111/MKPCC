...
int coreDecomposition(const MCSRGraph &g, int *core, int* seq, int* pos){
	...
	for (int u = 0; u < cg_maxvtx; u++){
		if (cg_valid[u]){
			for (int j = cg_pstart[u]; j < cg_pend[u]; j++){
				int neiu = cg_edges[j];
				if (u < neiu){
					triangles[j] = Utility::countIntersect(cg_edges + cg_pstart[u], cg_deg(u), cg_edges + cg_pstart[neiu], cg_deg(neiu));
				}
			}
		}
	}
